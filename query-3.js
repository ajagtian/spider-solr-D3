...
var y0 = d3.scale.linear().domain([0, 500]).range([height, 0]),
...
var svg = d3.select("#query-3").append("svg")
...
d3.tsv("data.tsv", type, function(error, data) {
  ...
  y0.domain([0, d3.max(data, function(d) { return d.Sales; })]);
  ...
  bars.append("rect")
	  ...
	  .attr("height", function(d,i,j) { return height - y1(d.Price); }); 
});
function type(d) {
  d.Sales = +d.Sales;
  return d;
